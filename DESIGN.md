# 金融新闻情报简报 Agent —— 系统设计文档

> 📌 **命名说明（2026-09-20）**：本项目原名「金融新闻驱动的股市**预测** Agent」，
> 已更名并重新定位为**情报简报**（详见 `README.md` 开头的说明）。
> 本文档正文沿用「预测」一词描述**回测与准确率检验**——那是在讲"当初设计的
> 是预测系统"，属设计记录，未逐字改写；**对外表述以 `README.md` 为准**。
...
- 用 APScheduler 配置 **cron 任务**：
  ...
  - 新闻增量采集：每 `60 分钟`（整点对齐，`NEWS_INTERVAL_MINUTES`）增量抓一次。
  - 板块数据采集：`17:40`（行业板块日频快照，走本地日缓存避免限流）。
  - 每日研判生成：`18:30`（2026-09-23 由 18:15 推后 —— 新闻轮 18:00 起跑要跑 21 分钟，
    18:15 取数会漏掉当天全部新闻）。
  - 周度综述生成：`19:30`，**仅在本周最后一个交易日**实际执行。
...
每小时 :00  Scheduler 触发 → 增量采集新闻 → 写入 news → 增量索引到 ChromaDB
18:30  生成研判 → LangGraph 流程 → 检索+分析 → 报告入库 report
