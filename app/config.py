...
class Settings(BaseSettings):
    ...
    # 主模型「失败冷却」（分钟，2026-09-23 新增）。
    #
    # **解决什么**：`with_fallbacks` **每次调用都从链首重来** —— 主模型挂了之后，
    # 每一批仍要**先白打一次主模型**才切到备用。实测 2026-09-23 那轮：
    # 第 27–34 批每批各打一次无用请求（8 次），日志里 8 条重复 WARNING。
    #
    # 现在的行为：主模型一旦失败就**冷却本值分钟**；冷却期内 `get_llm` 直接把
    # 主模型从链首摘掉，用第一个备用当主。到期后自动**半开**重试一次
    # （标准熔断三态：Closed → Open → Half-Open）。
    #
    # 取 15 分钟：覆盖一整轮（实测 11 分钟），又远小于「免费额度按天重置」的量级；
    # 到期后重试若仍失败，代价只是每 15 分钟一次白试（可忽略）。
    # **设 0 = 关闭**该行为（每次都从主模型试起，与改动前一致）。
    llm_primary_cooldown_minutes: float = 15.0
    ...
    # （行情 17:30 / 板块 17:40 / 日报 18:30 / 周报 19:30），而新闻采集每轮要跑
    ...
    # ---- P3 采集源健康监控（2026-09-22 新增）----
    # **为什么必需**：财联社端点**没有备用**（主备切换已取消，见设计 §8），
    # 所以「源失效」这件事**只剩下这一条发现路径**。没有它，财联社静默断流无人知晓。
    #
    # 陈旧度倍数：距上次**完整成功**超过 `倍数 × news_interval_minutes` 即告警。
    # 设计 §10 初稿写的是 1.5 倍；本实现取 **3**，因为一轮采集自己要跑 4~20 分钟，
    # 1.5 倍（90 分钟）只够跑 1~2 轮，偶发失败就会报 —— 而告警一旦变成日常噪声，
    # 真故障就会被淹掉。3 倍（180 分钟）既能滤掉噪声，也不至于半天才发现。
    news_stale_factor: float = 3.0
    # 连续「成功但 0 条」多少轮算异常。持续上升通常是**源改版 / 参数失效**的征兆 ——
    # 这是「源还活着但已经取不到东西」那类失效的唯一信号（它先于 404 出现）。
    news_empty_streak_alert: int = 3

    # 研判起跑前，若新闻采集轮**仍在运行**，最多等它这么多分钟（H1 的兜底，2026-09-24 新增）。
    #
    # **补的是什么**：把日报从 18:15 推到 18:30 只是**止血** —— 它给的是固定 30 分钟
    # 余量，而最坏情况（2400 条上限）约需 24.8 分钟，已吃掉 83%。LLM 一旦变慢
    # （例如主模型故障切到备用），余量立刻不够，而**那正是会产生 H1 的场景**。
    # 这里让研判**先等一下**再取数，比静默地少用一整天数据强。
    #
    # 等待有**硬上限**：研判自己也有时效要求，不能无界地等下去。超时后照常生成，
    # 由 `prepare_node` 如实标注「采集轮仍在运行，本轮新闻未纳入」。
    # **设 0 = 不等待**（行为与改动前一致）。
    report_wait_max_minutes: float = 5.0

    market_collect_time: str = "17:30"
    sector_collect_time: str = "17:40"   # 板块数据（在行情之后）
    # 2026-09-22 由 18:00 改为 18:15（原 18:00 正好压在 DeepSeek 高峰结束那一秒上）。
    # 2026-09-23 再由 18:15 改为 **18:30** —— 见下。
    #
    # **为什么推半小时**：新闻轮整点起跑（`minute="0"`），积压轮实测要跑 21 分钟
    # （2026-09-23 那轮 18:00:00 → 18:21:08），而日报 18:15 取数 —— **缺口 6 分 9 秒**，
    # 当天的新闻一条都没用上。改成 18:30 后有 30 分钟余量（最坏 2400 条约 24.8 分钟）。
    # ⚠️ 这是**止血不是根治**：采集量一旦超出（放宽 news_max_pages / LLM 变慢）
    # 仍会超时。真正的兜底是 `prepare_node` 里那两处如实标注（H1 的 D 步）。
    # 仍处 DeepSeek 空闲时段（高峰 9:00–12:00、14:00–18:00）✓
    # 与周报的间距：19:30 − 18:30 = 60 分钟，仍够用 ✓
