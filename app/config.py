...
class Settings(BaseSettings):
    ...
    # 2026-09-22 由 18:00 改为 18:15，与 `.env` / `.env.example` 对齐。
    # 原默认值 18:00 **正好压在 DeepSeek 高峰时段结束那一秒上**（高峰 9:00–12:00、
    # 14:00–18:00，其余半价）—— 不配 `.env` 的新部署会踩上去。
    report_time: str = "18:15"
    # 周报时间（仅每周最后一个交易日实际生成）。
