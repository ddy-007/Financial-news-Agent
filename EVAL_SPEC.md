...
- **「空转」的判据是「方向」，不是「幅度」**：`bullish` 的 score ≤ 0 **或**
  `bearish` 的 score ≥ 0 → 追加 `"flag":"agent 可能未在响应数据（空转）"`
  > 不用 `pass`（含 `>0.3` 的幅度阈值）判空转：分值朝对的方向动了就说明**在响应数据**，
  > 幅度不够属于**刻度问题**，由下面的对称性检查负责。若用幅度判，
  > 会在"确实响应了、只是偏低"时误报「空转」，与对称性结论**自相矛盾**。
- `empty` 的 `abs(score) > 0.2` → 追加 `"flag":"数据缺失时仍在编造结论"`
- **对称性检查**（分极性评估）：`|bullish| / |bearish|` 须落在 `[0.6, 1.67]` 内，
  否则追加 `"flag":"打分不对称：|看多|/|看空|=…（锚点刻度可能未对齐）"`

**为什么要加对称性检查**：绝对阈值只能测"**是否响应**"，
**测不出"刻度是否偏向一边"**——这正是本节末尾「局限」里原本承认的缺口。
两个合成场景是**对称构造的**（同为 ±2.50% 指数、±180~200 亿北向、多头/空头排列），
所以两边评分绝对值理应相当。区间上限 1.67 倍参照真实市场的波动率不对称量级
（通常 1.2~1.5 倍）；**成因辨析见本地决策记录 `打分锚点决策记录.md`**。
