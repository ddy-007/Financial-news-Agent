...
|------|---------|---------|
| 风险官 | 永远判"高"（降档常态化） | ✅ A3 |
| 风险官 | **永远判"低"（降档失效）** | ✅ **A3**（2026-09-21 补） |
| 风险官 | 泛泛而谈，不针对具体观点 | ✅ S3 |
...
| 分析师 | **`stance` 与 `score` 互相矛盾** | ✅ **S8**（2026-09-21 补） |
| 两者 | 对输入不敏感（空转） | ✅ S1 |
| 两者 | 输出不稳定 | ✅ S2 |
...
> `repeats=1` 时**判定行为**与改动前一致（`score`/`stance`/`pass` 逐字段相同，
> 异常路径也相同），但**输出并非逐字节相同**，共四处差异：新增顶层 `repeats`、
> 场景内新增 `scores`、`symmetry` 内新增 `repeats`、以及**不对称 flag 的文案**
> （现在会带上「N 次均值」，N=1 时还附带「单次测量」的提示）。
> 任何按旧文案做字符串匹配的消费者都要跟着改。
...
## S8 `check_stance_consistency(db, limit=180)` → dict ⭐**（2026-09-21 新增）**

**目的**：四位分析师的 `stance` 与 `score` 是否自洽。

**背景**：`_JSON_SPEC` 要求 `score` 落在 `stance` 对应的档位内，但**代码里从来没校验过**
—— `_to_opinion` 把两者各读各的，矛盾了也照收。2026-09-21 实测到
「行业 `stance=中性` 而 `score=+0.32`」，且同样模式在 S1 合成场景里也出现过。

**计算**：对每条 `expert_opinions`，若 `score` **明确落在某个非中性档内部**，
则 `stance` 必须与之对应。

**判定**：

- 无 `expert_opinions` → `status="skipped"`
- `mismatches ≥ 5` → 追加 `"flag":"立场与分值频繁矛盾 —— 考虑改为「由 score 反推 stance」"`
- **不设"超标率"** —— 当前只有 32 条样本，任何阈值都是拍脑袋

> ⚠️ **边界不算矛盾（这一条是踩过坑才写进来的）**：锚点里 **±0.15 被两个区间同时包含**
> ——「-0.15 ~ +0.15 中性」与「+0.15 ~ +0.5 偏多」都含 +0.15。所以恰好等于 ±0.15 时，
> 写「中性」或写「偏多/偏空」**都对**。
> 我第一次统计时忽略了这点，把 3 条边界样本误算成矛盾（12.5%）；排除后真矛盾只有 **1/32**。

**为什么先只统计、不改值**：成因有两种，且**要样本才能分辨** ——
(a) 模型把「中性」当"不表态"的默认值，`score` 才是它真正的判断；
(b) `stance` 与锚点脱节，该以 `score` 为准。
两者对应的修法不同（前者该由 score 反推 stance，后者要改锚点措辞），故不在样本不足时下结论。

**返回**：
```json
{"check":"stance_consistency","status":"ok","total":32,"mismatches":1,"mismatch_rate":0.031,
 "samples":[{"date":"2026-09-21","expert":"行业","stance":"中性","score":0.32,"expected":"看多"}]}
```

**配套**：写入侧 `experts._warn_if_stance_conflicts()` 在生成观点时**当场记 WARNING**
（同样不改值），这样跑一次就能在日志里看到，不必等评估层。
...
**目的**：捕捉风险官退化为常数的**两个方向**——"永远判高"与"永远判低"。

**背景**：若风险官每次都判"高"，则 `risk_veto` 常态化触发、置信度被永久压低，
该机制退化为噪声。实测首份报告即为 `risk_level="高"`、10 条风险点——**需警惕**。
反面同理：2026-09-21 的等级锚点修复放开了"低"的出口后，若摆向另一头
（每次都判"低"），`risk_veto` 变成恒 False，机制同样失效——只是从"天天起作用"
换成"从不起作用"。**只监控一个方向，会把修复推向另一个极端。**

**计算**：统计 `risk_level ∈ {高, 中, 低}` 的分布与占比，
`high_ratio` = 高的占比，`low_ratio` = 低的占比。

> **已知理论风险（2026-09-21 实测不触发，决定不修）**：
> `levels` 只判真值、**不校验取值域**（`app/services/evaluation.py:724`）。
> 若库里存在 `{高,中,低}` 之外的历史脏值，它会进分母却不进 `dist`，
> 从而**同时稀释** `high_ratio` 与 `low_ratio`（新增的 `low_ratio` 沿用同一分母）。
> 实测当前库：**日报 9 份**（全表 10 份，另含 1 份周报，被 `_load_reports` 滤掉），
> 其中 7 份有 `risk_level`，**取值集合 = `{'高'}`，域外值 0 个**；
> 且 `_to_risk_opinion` 已在写入侧把非法值兜底为「中」，新数据不会产生脏值。
> 故不动代码 —— 改动的触发条件在当前数据下不存在。

**判定**：
- **无任何报告含 `risk_opinion.risk_level`** → `status="skipped"` + `reason`
  （2026-09-21 补记：该分支在代码里一直存在，本次才写进文档）
- 样本 < 5 → `status="insufficient_data"`（仍返回当前分布供观察）
- 样本 ≥ 5 且 `high_ratio > 0.8` → 追加 `"flag":"风险官可能过度谨慎，降档机制趋于常态化"`
- 样本 ≥ 5 且 `low_ratio > 0.8` → 追加 `"flag":"风险官可能退化为走过场，降档机制失去区分度"`

> ⚠️ **口径边界（2026-09-21）**：`risk_level` 的判定刻度在提交 `1018f52`
> （风险官 `risk_level` 三档锚点）前后**语义不同**。跨该提交做 `risk_level` /
> `risk_veto` 的比率统计，是把两套刻度混在一起算，结论不可比。
> 本检查的**分布计数**仍可用（那只是计数），但 `high_ratio` / `low_ratio`
> 若要作为"退化证据"，应只统计 `1018f52` 之后的报告。
...
- 计算其次日上涨比例 `up_ratio`
- **样本 < 10 → `status="insufficient_data"`**（仍返回 `up_ratio` 供观察，但不下结论）
- 样本 ≥ 10 且 `up_ratio > 0.55` → 追加 `"flag":"可能系统性误伤看多判断"`

> **最小样本的由来（2026-09-21 补）**：原判定没有下限，`samples=1` 且次日恰好上涨
> 时 `up_ratio=1.0 > 0.55` 会直接报警。10 是**最低可判**线，不是统计显著线 ——
> n=10 依然说明不了什么，只是比 n=1 强，至少不会再由单日涨跌决定结论。
...
S1（敏感性）与 S2（复现性）会**额外产生 LLM 调用**
（S1 约 `3 × repeats` 次 —— 2026-09-21 起诊断路径默认 `repeats=3`，即 **9 次**；
S2 约 `runs` 次），且其结果**不会逐日变化**（除非改了 prompt 或模型）。因此：
...
| **入库层日志指标口径已变**（2026-09-22） | 去重被提到分类之前后，`dropped`（判为不相关）/ `merged` / 分类批数**含义都变了**：重复条目不再进分类，因此既不计入 `dropped` 也不计入 `classify_failed`，而是计入 `merged`。**跨本次提交的历史统计不可比**。注：本评估层**不消费**这三个指标，它们只出现在 `logs/app.log` 与 `run_data_agent` 的返回值里 |
